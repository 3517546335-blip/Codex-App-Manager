    ...
    match event {
        ShellEvent::ConfirmQuit => {
            let mut dialog = app
                .dialog()
                .message(
                    "The interface is not responding. Quit Codex Desktop 管理器 safely?\n\n\
                     界面没有响应。是否安全退出 Codex Desktop 管理器？",
                )
                .title(PRODUCT_NAME)
        ...
        ShellEvent::QuitBlocked(policy) => {
            ...
            let mut dialog = app
                .dialog()
                .message(format!(
                    "Codex Desktop 管理器 must stay open until the protected step finishes.\n\
                     {reason}\n\n受保护步骤完成前，Codex Desktop 管理器 必须保持打开。"
                ))
                .title(PRODUCT_NAME)
                .kind(MessageDialogKind::Warning)
...
fn show_windows_startup_error(detail: &str) {
    ...
    let title: Vec<u16> = "Codex Desktop 管理器 startup error\0".encode_utf16().collect();
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            let offline_root = ["resources/offline", "offline"]
                .into_iter()
                .filter_map(|rel| {
                    app.path()
            ...
            if let Some(root) = offline_root {
                crate::app::offline_bundle::initialize(root)?;
            } else {
                log::warn!("embedded offline Codex resources were not found");
            }
            build_main_window(app)?;
            #[cfg(target_os = "macos")]
            install_macos_menu(app.handle(), NativeLocale::En)?;
            log::info!(
                "Codex Desktop 管理器 v{} starting (os={}, arch={})",
                app.package_info().version,
                std::env::consts::OS,
                std::env::consts::ARCH
            );
        ...
        .expect("failed to build Codex Desktop 管理器")
