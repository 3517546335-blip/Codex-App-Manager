...
use codex_win_engine::{
    ...
    fetch_text_timeout_with_network, fetch_text_with_network, find_msix_sha256,
...
use crate::app::mirror_fallback::{is_retryable_mirror_error, r2_fallback_url};
...
fn normalize_windows_architecture(value: &str) -> Option<&'static str> {
...
fn validate_official_windows_source(
    text: &str,
    release: &WindowsRelease,
) -> Result<String, AppError> {
    let source: OfficialWindowsSource = serde_json::from_str(text)
        .map_err(|e| AppError::Engine(format!("invalid Windows official source JSON: {e}")))?;
    if source.schema_version != 1 {
        return Err(AppError::Engine(format!(
            "unsupported Windows official source schema {}",
    ...
    let release_arch = release
        ...
        .as_deref()
        .or(release.architecture.as_deref())
        .and_then(normalize_windows_architecture);
    if release_arch != Some("x64")
        || normalize_windows_architecture(&source.architecture) != Some("x64")
    {
        return Err(AppError::Engine(
            "Windows official source is only available for x64".to_string(),
        ));
    }
    if source.store_product_id != "9PLM9XGG6VKS"
        || release
            .store_product_id
            .as_deref()
            .is_some_and(|id| id != source.store_product_id.as_str())
        || source.package_identity != codex_win_engine::OPENAI_PACKAGE_IDENTITY
        || release
            .package_identity
            .as_deref()
            .is_some_and(|id| id != source.package_identity.as_str())
        || source.version != release.version
        || source.package_version != release.package_version
        || source.package_moniker != release.package_moniker
        || Some(source.content_length) != release.content_length
    {
        return Err(AppError::Engine(
            "Windows official source does not match the trusted mirror manifest".to_string(),
        ));
    }

    let url = url::Url::parse(&source.url)
        .map_err(|e| AppError::Engine(format!("invalid Microsoft package URL: {e}")))?;
    ...
    {
        return Err(AppError::Engine(
            "Windows official source returned an untrusted Microsoft package URL".to_string(),
        ));
    }
    let expires_at = url
        .query_pairs()
        ...
        .ok_or_else(|| AppError::Engine("Microsoft package URL has no valid expiry".to_string()))?;
    let now = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs();
    if expires_at <= now.saturating_add(60) {
        return Err(AppError::Engine(
            "Microsoft package URL is expired or too close to expiry".to_string(),
        ));
    }
    Ok(url.into())
}

fn official_windows_package_url(
    release: &WindowsRelease,
    ...
    let cache_buster = SystemTime::now()
        ...
        .as_secs()
        / 60;
    let descriptor_url = format!("{WINDOWS_OFFICIAL_SOURCE_URL}?v={cache_buster}");
    let text = fetch_text_timeout_with_network(&descriptor_url, 15, network).map_err(engine_err)?;
    validate_official_windows_source(&text, release)
}

fn download_verified_windows_candidate(
    url: &str,
    dest: &Path,
    expected_size: u64,
    expected_sha: &str,
    ...
    let actual_size = std::fs::metadata(dest)
        .map_err(|e| AppError::Engine(format!("read downloaded MSIX metadata: {e}")))?
        .len();
    if expected_size > 0 && actual_size != expected_size {
        let _ = std::fs::remove_file(dest);
        return Err(AppError::Engine(format!(
            "MSIX size mismatch from {source}: {actual_size} != {expected_size}"
        )));
    }
    ...
    if !actual_sha.eq_ignore_ascii_case(expected_sha) {
        let _ = std::fs::remove_file(dest);
        return Err(AppError::Engine(format!(
            "MSIX sha256 mismatch from {source}: {actual_sha} != {expected_sha}"
        )));
    }
    Ok(source)
}

fn download_windows_artifact_with_fallbacks(
    primary_url: &str,
    release: &WindowsRelease,
    dest: &Path,
    expected_size: u64,
    ...
    let attempt = |url: &str| {
        download_verified_windows_candidate(
            url,
            dest,
            expected_size,
            expected_sha,
    ...
    let primary_err = match attempt(primary_url) {
        Ok(source) => return Ok(source),
        Err(err) => err,
    };
    if primary_err.to_string().contains("download cancelled") {
        return Err(primary_err);
    }
    let Some(r2_url) = r2_fallback_url(primary_url) else {
        return Err(primary_err);
    };
    let primary_retryable = is_retryable_mirror_error(&primary_err.to_string())
        || primary_err.to_string().contains("mismatch");
    if !primary_retryable {
        return Err(primary_err);
    }

    let official_failure = match official_windows_package_url(release, network) {
        Ok(official_url) => {
            log::warn!(
                "Windows primary mirror unavailable; trying Microsoft CDN primary={} official={} error={primary_err}",
                host_of(primary_url),
                host_of(&official_url),
            );
            match attempt(&official_url) {
                Ok(source) => return Ok(source),
                Err(err) if err.to_string().contains("download cancelled") => return Err(err),
                Err(err) => err,
            }
        }
        Err(err) => err,
    ...
    log::warn!(
        "Windows Microsoft CDN unavailable; trying R2 primary={} fallback={} official_error={}",
        host_of(primary_url),
        host_of(&r2_url),
        official_failure,
    );
    attempt(&r2_url).map_err(|r2_err| {
        AppError::Engine(format!(
            "primary mirror failed ({primary_err}); Microsoft CDN failed ({}); R2 failed ({r2_err})",
            official_failure
        ))
    })
}
...
fn r2_fallback_endpoints(endpoints: &MirrorEndpoints) -> Option<MirrorEndpoints> {
    Some(MirrorEndpoints {
        manifest_url: r2_fallback_url(&endpoints.manifest_url)?,
        checksums_url: r2_fallback_url(&endpoints.checksums_url)?,
        windows_msix_url: r2_fallback_url(&endpoints.windows_msix_url)?,
        windows_x64_msix_url: r2_fallback_url(&endpoints.windows_x64_msix_url)?,
        windows_arm64_msix_url: r2_fallback_url(&endpoints.windows_arm64_msix_url)?,
        windows_unpacked_url: r2_fallback_url(&endpoints.windows_unpacked_url)?,
        mac_arm64_url: r2_fallback_url(&endpoints.mac_arm64_url)?,
        mac_intel_url: r2_fallback_url(&endpoints.mac_intel_url)?,
    })
}

fn read_windows_release_once(
    endpoints: &MirrorEndpoints,
    network: &NetworkConfig,
) -> Result<(WindowsRelease, String, String, String, String), AppError> {
    let manifest_text =
    ...
    Ok((
        ...
        sha256,
        package_url,
        endpoints.manifest_url.clone(),
        endpoints.checksums_url.clone(),
    ))
}
...
fn read_windows_release(
    ...
    match read_windows_release_once(endpoints, network) {
        Ok(release) => Ok(release),
        Err(primary_err) if is_retryable_mirror_error(&primary_err.to_string()) => {
            let Some(fallback) = r2_fallback_endpoints(endpoints) else {
                return Err(primary_err);
            };
            log::warn!(
                "Windows primary metadata unavailable; trying R2 primary={} fallback={} error={primary_err}",
                host_of(&endpoints.manifest_url),
                host_of(&fallback.manifest_url),
            );
            let (release, sha256, _, manifest_url, checksums_url) = read_windows_release_once(
                &fallback, network,
            )
            .map_err(|fallback_err| {
                AppError::Engine(format!(
                    "primary metadata failed ({primary_err}); R2 metadata failed ({fallback_err})"
                ))
            })?;
            let package_url = endpoints
                .windows_msix_url_for_arch(release.download_architecture.as_deref())
                .to_string();
            Ok((release, sha256, package_url, manifest_url, checksums_url))
        }
        Err(err) => Err(err),
    }
}
...
fn remove_missing_windows_apps_records(store: &mut ProvenanceStore) -> bool {
    let original_len = store.managed.len();
    store.managed.retain(|record| {
        if !is_windows_apps_path(&record.path) {
            return true;
        }
        match Path::new(&record.path).try_exists() {
            Ok(false) => {
                log::info!(
                    "remove stale WindowsApps provenance path={} source={}",
                    record.path,
                    record.source
                );
                false
            }
            // AccessDenied cannot prove that a protected WindowsApps path is
            // gone, so keep the record rather than discarding valid history.
            Ok(true) | Err(_) => true,
        }
    });
    store.managed.len() != original_len
}

fn detect_windows_localization_target(
    settings: &AppSettings,
    install_mode: &str,
    store: &ProvenanceStore,
) -> Option<InstalledWindowsCodex> {
    if install_mode == "portable" {
        let root = PathBuf::from(&settings.install_root);
        if let Some(portable) = detect_portable_install(&root) {
            return Some(portable);
        }
    }
    detect_managed_codex(settings, store)
}

fn register_selected_portable(
    store: &mut ProvenanceStore,
    portable: &InstalledWindowsCodex,
) -> bool {
    let build = version_key(&portable.version);
    if store.is_managed_build(&portable.path, build) {
        return false;
    }
    store.record(portable.path.clone(), build, "adopted-external");
    true
}

/// Resolve the Windows install modified by one-click localization. An explicit,
/// valid portable root wins even before it has a provenance record; clicking
/// one-click configuration is also sufficient consent to adopt that target.
pub fn resolve_windows_localization_target(
    settings: &AppSettings,
    install_mode: &str,
) -> Result<Option<InstalledWindowsCodex>, AppError> {
    let mut store = ProvenanceStore::load();
    let mut store_changed = remove_missing_windows_apps_records(&mut store);
    let installed = detect_windows_localization_target(settings, install_mode, &store);
    let mut registration_required = false;

    if install_mode == "portable" {
        if let Some(portable) = installed.as_ref().filter(|item| item.source == "portable") {
            let build = version_key(&portable.version);
            if register_selected_portable(&mut store, portable) {
                log::info!(
                    "adopt selected portable install before Windows localization path={} build={build}",
                    portable.path
                );
                store_changed = true;
    ...
    if store_changed {
        if let Err(err) = store.save() {
            ...
            log::warn!("could not persist stale WindowsApps provenance cleanup error={err}");
        }
    }
    Ok(installed)
}
...
pub fn plan_windows_update_with_install_mode_and_network(
    ...
    let (release, sha256, package_url, manifest_url, checksums_url) =
        read_windows_release(endpoints, network)?;
...
pub fn stage_windows_update_with_install_mode_and_network(
    ...
    let stage_result = (|| -> Result<WinStageReport, AppError> {
        ...
        let mut source = host_of(&report.package_url);
        let mut downloaded_sha = None;
        ...
        if !cached_ok {
            ...
            source = download_windows_artifact_with_fallbacks(
                &report.package_url,
                &report.release,
                &dest,
                expected_size,
            ...
            downloaded_sha = Some(expected_sha.clone());
        }
        ...
        let actual_sha = match downloaded_sha {
            Some(sha) => sha,
            None => sha256_file(&dest).map_err(engine_err)?,
        };
        if !actual_sha.eq_ignore_ascii_case(&expected_sha) {
            return Err(AppError::Engine(format!(
...
pub fn launch_codex(settings: &AppSettings) -> Result<(), AppError> {
    ...
    let manager_data_dir = crate::app::paths::data_dir()
        .unwrap_or_else(|| std::env::temp_dir().join("codex-app-manager"));
    let launch_target = codex_win_engine::patched_chinese_install(&installed, &manager_data_dir)
        .unwrap_or(installed);
    let path = &launch_target.path;
    log::info!(
        "Windows launch Codex path={path} source={}",
...
mod tests {
    use super::{
        ...
        detect_managed_codex, detect_windows_localization_target,
        outcome_from_portable_uninstall, portable_boundary_mutated_install_root,
        r2_fallback_endpoints, register_selected_portable, remove_missing_windows_apps_records,
        retry_windows_ancillary_with_detector, validate_official_windows_source, WinAbortGuard,
        WinInstallStatus, WinPerformAction, WIN_UPDATE_ABORT,
    };
  ...
  <Identity Name="OpenAI.Codex" Publisher="CN=OpenAI OpCo, LLC" Version="{version}" ProcessorArchitecture="x64" />
    ...
    #[test]
    fn localization_prefers_explicit_portable_without_provenance() {
        let selected_dir = temp_test_dir("localization-selected-portable");
        let recorded_dir = temp_test_dir("localization-recorded-portable");
        write_fake_portable_install(&selected_dir, "26.803.41515.0");
        write_fake_portable_install(&recorded_dir, "26.803.5235.0");

        let mut store = ProvenanceStore::default();
        store.record(
            recorded_dir.to_string_lossy().into_owned(),
            codex_win_engine::version_key("26.803.5235.0"),
            "manager-installed-portable",
        );
        let settings = AppSettings::new(
            "https://codexapp.agentsmirror.com".to_string(),
        ...
        let installed =
            detect_windows_localization_target(&settings, "portable", &store).unwrap();
        assert_eq!(installed.path, selected_dir.to_string_lossy());
        assert_eq!(installed.source, "portable");
        assert!(!store.is_managed(&installed.path));

        assert!(register_selected_portable(&mut store, &installed));
        assert!(store.is_managed_build(
            &installed.path,
            codex_win_engine::version_key(&installed.version)
        ));
        assert!(!register_selected_portable(&mut store, &installed));
    ...
    #[test]
    fn localization_prunes_only_proven_missing_windows_apps_records() {
        let root = temp_test_dir("stale-windows-apps-record");
        let stale = root.join("WindowsApps").join("OpenAI.Codex_missing");
        let ordinary_missing = root.join("portable-may-return");
        let mut store = ProvenanceStore::default();
        store.record(
        ...
        store.record(
            ordinary_missing.to_string_lossy().into_owned(),
            2,
            "manager-installed-portable",
        );

        assert!(remove_missing_windows_apps_records(&mut store));
        assert_eq!(store.managed.len(), 1);
        assert_eq!(store.managed[0].path, ordinary_missing.to_string_lossy());
    ...
    #[test]
    fn accepts_only_a_current_matching_microsoft_delivery_descriptor() {
        ...
        let descriptor = serde_json::json!({
            ...
            "packageMoniker": release.package_moniker,
            "contentLength": release.content_length,
            "url": format!(
                "http://tlu.dl.delivery.mp.microsoft.com/filestreamingservice/files/id?P1={expires}&P2=404"
            )
        });
        let url = validate_official_windows_source(&descriptor.to_string(), &release).unwrap();
        assert!(url.starts_with("http://tlu.dl.delivery.mp.microsoft.com/"));

        let mut wrong_version = descriptor.clone();
        wrong_version["packageVersion"] = serde_json::json!("99.0.0.0");
        assert!(validate_official_windows_source(&wrong_version.to_string(), &release).is_err());

        let mut wrong_host = descriptor;
        wrong_host["url"] = serde_json::json!(format!(
            "http://dl.delivery.mp.microsoft.com.evil.example/filestreamingservice/files/id?P1={expires}"
        ));
        assert!(validate_official_windows_source(&wrong_host.to_string(), &release).is_err());
    }

    #[test]
    fn r2_fallback_is_available_only_for_the_built_in_primary_mirror() {
        let primary = crate::domain::manifest::MirrorEndpoints::from_base_url(
            "https://codexapp.agentsmirror.com",
        );
        let fallback = r2_fallback_endpoints(&primary).unwrap();
        assert!(fallback
            .windows_x64_msix_url
            .starts_with("https://codexapp-r2.agentsmirror.com/"));

        let custom = crate::domain::manifest::MirrorEndpoints::from_base_url(
            "https://mirror.example.test",
        );
        assert!(r2_fallback_endpoints(&custom).is_none());
    }
