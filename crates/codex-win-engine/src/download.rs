...
fn copy_local_file(
    ...
    let source_size = std::fs::metadata(source)
        .map_err(|e| format!("read embedded package metadata: {e}"))?
        .len();
    if source_size > max_bytes {
        return Err(format!(
            "embedded package exceeds size limit: {source_size} > {max_bytes}"
        ));
    }
    let mut input = std::fs::File::open(source)
        .map_err(|e| format!("open embedded package {}: {e}", source.display()))?;
    let mut output = std::fs::File::create(dest)
        .map_err(|e| format!("create local staging file {}: {e}", dest.display()))?;
    ...
    loop {
        ...
        let read = input
            .read(&mut buffer)
            .map_err(|e| format!("read embedded package: {e}"))?;
        if read == 0 {
            break;
        }
        output
            .write_all(&buffer[..read])
            .map_err(|e| format!("copy embedded package: {e}"))?;
        copied = copied.saturating_add(read as u64);
        on_progress(copied);
    }
    output
        .sync_all()
        .map_err(|e| format!("flush embedded package staging file: {e}"))?;
    if copied != source_size {
        return Err(format!(
            "embedded package copy size mismatch: {copied} != {source_size}"
        ));
    }
    Ok(())
}
...
fn is_microsoft_delivery_url(url: &str) -> bool {
    ...
    matches!(parsed.scheme(), "http" | "https")
        && (host.eq_ignore_ascii_case("dl.delivery.mp.microsoft.com")
            || host
                .to_ascii_lowercase()
                .ends_with(".dl.delivery.mp.microsoft.com"))
        && parsed.path().starts_with("/filestreamingservice/files/")
...
fn curl_args(
    ...
    if is_microsoft_delivery_url(url) {
        let protocol = format!("={}", url::Url::parse(url).unwrap().scheme());
        args.extend([
            "-f".to_string(),
            "--proto".to_string(),
            protocol.clone(),
...
pub fn download_to_with_progress_bounded_with_network(
    ...
    let local_source = local_file_path(url);
    let should_resume = local_source.is_none()
        && part.metadata().map(|m| m.len() > 0).unwrap_or(false);
    let source = url_host(url);
    let dest_name = dest
        .file_name()
    ...
    let download_result = match local_source.as_deref() {
        Some(source) => copy_local_file(source, &part, max_bytes, on_progress),
        None => run_curl(url, &part, should_resume, max_bytes, on_progress, network),
    };
...
mod tests {
    ...
    #[test]
    fn allows_only_direct_microsoft_delivery_http_urls() {
        let trusted = curl_args(
            "http://tlu.dl.delivery.mp.microsoft.com/filestreamingservice/files/id?P1=1",
            "Codex.msix.part",
            false,
            "123",
            ProgressMode::NoProgressMeter,
        ...
        assert!(trusted.windows(2).any(|pair| pair == ["--proto", "=http"]));
        assert!(trusted.windows(2).any(|pair| pair == ["--max-redirs", "0"]));
        assert!(!trusted.contains(&"-fL".to_string()));

        for untrusted in [
            "http://evil.example/filestreamingservice/files/id",
            "http://dl.delivery.mp.microsoft.com.evil.example/filestreamingservice/files/id",
            "http://user@tlu.dl.delivery.mp.microsoft.com/filestreamingservice/files/id",
            "http://tlu.dl.delivery.mp.microsoft.com/other/id",
        ] {
            let args = curl_args(
                untrusted,
                "Codex.msix.part",
