...
fn fetch_text_output(
    ...
    run_capturing(
        command,
        RunLimits::total(std::time::Duration::from_secs(timeout_seconds + 15)),
        None,
    )
...
pub fn fetch_text_with_network(url: &str, network: &NetworkConfig) -> Result<String, EngineError> {
    fetch_text_timeout_with_network(url, 60, network)
}
...
pub fn fetch_text_timeout_with_network(
    ...
    if let Ok(parsed) = url::Url::parse(url) {
        if parsed.scheme() == "file" {
            let path = parsed.to_file_path().map_err(|_| {
                EngineError::Io("invalid embedded metadata file URL".to_string())
            })?;
            let metadata = std::fs::metadata(&path)
                .map_err(|e| EngineError::Io(format!("read embedded metadata: {e}")))?;
            if metadata.len() > MAX_TEXT_BYTES {
                return Err(EngineError::Io(format!(
                    "embedded metadata exceeded {MAX_TEXT_BYTES} bytes"
                )));
            }
    ...
    let timeout_seconds = timeout_seconds.clamp(1, 60);
    let source = url_host(url);
    log::debug!("fetch Windows text source={source}");
    let mut output = fetch_text_output(
        url,
        network,
    ...
    if should_retry_without_revocation {
        ...
        output = fetch_text_output(
            url,
            network,
            SchannelRevocationCheck::Disabled,
            timeout_seconds,
        )?;
    }
...
fn managed_chinese_user_data_path(
    root: &Path,
    package_family_name: Option<&str>,
    local_app_data: Option<&std::ffi::OsStr>,
) -> PathBuf {
    if let (Some(family), Some(local_app_data)) = (package_family_name, local_app_data) {
        let package_user_data = PathBuf::from(local_app_data)
            .join("Packages")
            .join(family)
            .join("LocalCache")
            .join("Roaming")
            ...
            .join("web")
            .join("Codex");
        if package_user_data.is_dir() {
            return package_user_data;
        }
    }
    root.parent().unwrap_or(root).join("electron-user-data")
}
...
pub fn launch_codex_with_options(
    ...
    if installed.source == "portable" {
        ...
        let managed_chinese_copy = root
            .parent()
            .is_some_and(|parent| parent.join(".codex-manager-zh-cn.json").is_file());
        ...
        if managed_chinese_copy {
            // Electron forwards a second launch to the existing English MSIX
            // and exits with code 0, so release the global instance first.
            crate::windows_process::force_close_known_codex_processes()?;
            let _ = close_msix_codex_processes(3);
            let _ = crate::windows_process::close_codex_processes_for_root(2, root);
        }
        ...
        if crate::chinese_patch::codex_chinese_patch_is_active(installed) {
            command.arg("--lang=zh-CN");
        }
        if managed_chinese_copy {
            if let Some(user_profile) = std::env::var_os("USERPROFILE") {
                command.env("CODEX_HOME", PathBuf::from(user_profile).join(".codex"));
            }
            let cli = root.join("resources").join("codex.exe");
            if cli.is_file() {
                command.env("CODEX_CLI_PATH", cli);
            }
            command.env(
                "CODEX_ELECTRON_USER_DATA_PATH",
                managed_chinese_user_data_path(
                    root,
        ...
        match spawn_and_require_liveness(command, PORTABLE_LIVENESS_WINDOW) {
            ...
            Ok(LivenessResult::ExitedEarly { code }) => {
                // Electron can replace its bootstrap process with another
                // process from the same app tree. Accept that handoff only if
                // the Chinese copy is actually still running.
                std::thread::sleep(std::time::Duration::from_millis(350));
                if crate::windows_process::codex_processes_running_for_root(root)
                    .unwrap_or(false)
                {
                    Ok(())
                } else {
                    Err(EngineError::Io(format!(
                        "Codex exited immediately after launch (exit={}); another ChatGPT/Codex process may still hold the single-instance lock",
...
mod tests {
    ...
    use super::*;
    use super::{
        fetch_text_timeout_with_network, managed_chinese_user_data_path, registered_msix_aumid,
        remote_debugging_arguments,
    };

    #[test]
    fn reads_embedded_metadata_from_file_url() {
        let path = std::env::temp_dir().join(format!(
            "codex-offline-metadata-{}-{}.json",
            std::process::id(),
            uuid::Uuid::new_v4()
        ));
        std::fs::write(&path, br#"{"schemaVersion":2}"#).unwrap();
        let url = url::Url::from_file_path(&path).unwrap().to_string();
        let text = fetch_text_timeout_with_network(&url, 1, &crate::NetworkConfig::direct())
            .expect("read local metadata");
        assert_eq!(text, r#"{"schemaVersion":2}"#);
        let _ = std::fs::remove_file(path);
    }
    ...
    #[test]
    fn managed_chinese_copy_reuses_existing_store_user_data() {
        let root =
            std::env::temp_dir().join(format!("codex-managed-user-data-{}", std::process::id()));
        let app = root.join("patched").join("app");
    ...
    #[test]
    fn managed_chinese_copy_has_private_user_data_fallback() {
        let version_root = std::env::temp_dir()
            .join("codex-manager")
            .join("chinese-patch")
            .join("apps")
            .join("v1");
        let app = version_root.join("app");
        assert_eq!(
            managed_chinese_user_data_path(&app, Some("OpenAI.Codex_test"), None),
            version_root.join("electron-user-data")
        );
    }
