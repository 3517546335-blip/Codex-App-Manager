...
const KNOWN_CODEX_PROCESS_NAMES: &[&str] = &[
    "ChatGPT.exe",
    "Codex.exe",
    "codex-helper.exe",
    "CodexHelper.exe",
...
#[cfg(windows)]
pub fn force_close_known_codex_processes() -> Result<(), EngineError> {
    use std::os::windows::process::CommandExt;
    use std::process::{Command, Stdio};

    const CREATE_NO_WINDOW: u32 = 0x0800_0000;

    for name in KNOWN_CODEX_PROCESS_NAMES {
        let status = Command::new("taskkill.exe")
            ...
            .stderr(Stdio::null())
            .status()
            .map_err(|e| EngineError::Install(format!("force-close {name}: {e}")))?;
        if !status.success() {
            // taskkill returns non-zero when the requested image is absent.
            // The root-scoped close that follows verifies actual leftovers.
            log::debug!("taskkill found no closable process image={name} status={status}");
        }
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn force_close_targets_current_codex_entrypoints() {
        assert!(KNOWN_CODEX_PROCESS_NAMES.contains(&"ChatGPT.exe"));
        assert!(KNOWN_CODEX_PROCESS_NAMES.contains(&"Codex.exe"));
        assert!(KNOWN_CODEX_PROCESS_NAMES.contains(&"codex-helper.exe"));
    }
