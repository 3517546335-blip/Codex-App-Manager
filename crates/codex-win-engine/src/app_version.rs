...
/// Locate the Electron payload archive under a detected Codex install root.
///
/// Both registered MSIX layouts and the manager's portable layout are
/// supported. The bounded fallback walk covers package-layout changes without
/// searching outside the detected install root.
pub fn find_codex_app_asar(root: &Path) -> Option<PathBuf> {
