...
fn copy_local_file(
    ...
    let source_size = std::fs::metadata(source)
        .map_err(|e| EngineError::Io(format!("read embedded package metadata: {e}")))?
        .len();
    ...
    let mut input = std::fs::File::open(source)
        .map_err(|e| EngineError::Io(format!("open embedded package: {e}")))?;
    let mut output = std::fs::File::create(dest)
        .map_err(|e| EngineError::Io(format!("create local staging file: {e}")))?;
    ...
    loop {
        ...
        let read = input
            .read(&mut buffer)
            .map_err(|e| EngineError::Io(format!("read embedded package: {e}")))?;
        if read == 0 {
            break;
        }
        output
            .write_all(&buffer[..read])
            .map_err(|e| EngineError::Io(format!("copy embedded package: {e}")))?;
        copied = copied.saturating_add(read as u64);
        on_progress(copied);
    }
    output
        .sync_all()
        .map_err(|e| EngineError::Io(format!("flush embedded package staging file: {e}")))?;
