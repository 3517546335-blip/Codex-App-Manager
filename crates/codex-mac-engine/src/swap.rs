...
/// Wait until the exact app bundle's main process has remained alive briefly.
/// `open` only confirms that LaunchServices accepted a request; an app that
/// crashes during startup can otherwise be reported as successfully launched.
pub fn wait_for_codex_running(install_app: &Path, timeout: std::time::Duration) -> bool {
    ...
    loop {
        if codex_running_at(install_app) {
            ...
            // The process appeared but died before it was stable.
            return false;
        }
...
pub fn relaunch(install_app: &Path) -> Result<(), EngineError> {
    ...
    if !wait_for_codex_running(install_app, std::time::Duration::from_secs(10)) {
        let err = EngineError::Io(
            "macOS accepted the open request, but the Codex process did not stay running"
                .to_string(),
        );
        log::warn!("Codex relaunch failed error={err}");
        return Err(err);
