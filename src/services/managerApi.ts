...
export const MANAGER_UPDATE_GUIDE_URL =
  "https://rcn0zurrrxnx.feishu.cn/wiki/KmhFwc1fLi7QhJkSVawc6vyhnPg";
...
export interface ProviderConfigReport {
  ...
  addedKeys: string[];
  message: string;
  localization: ProviderLocalizationReport | null;
}

export interface ProviderLocalizationReport {
  status: "patched" | "already-patched" | "not-installed" | "failed" | string;
  message: string;
...
export const managerApi = {
  ...
  applyProviderConfig(): Promise<ProviderConfigReport> {
    if (!hasTauriRuntime()) {
      return Promise.resolve({
        ...
        addedKeys: [
          ...
          "model_providers.donaapi.name",
          "model_providers.donaapi.base_url",
          "model_providers.donaapi.wire_api",
          "model_providers.donaapi.requires_openai_auth",
        ],
        message: "浏览器开发态：会重写 DonaAPI 模型配置。桌面 app 内写配置不需要管理员权限。",
        localization: null,
      });
    }
    return invoke<ProviderConfigReport>("apply_provider_config");
  },
