...
const DONA_API_URL = "https://donaapi.com";

export function SpeedApiBanner() {
  return (
    <button
      type="button"
      className="speedapi-banner"
      title={DONA_API_URL}
      onClick={() => void managerApi.openUrl(DONA_API_URL)}
    >
      <span className="speedapi-copy">
        <span className="speedapi-kicker">让每一次 AI 调用更简单</span>
        <span className="speedapi-url">{DONA_API_URL}</span>
      </span>
      <Icon name="external" className="speedapi-external" />
    </button>
  );
}
