...
type Busy = "plan" | "perform" | "adopt" | "install" | "launch" | "config" | null;
...
export function WinHome({
  ...
  const applyOneClickConfig = useCallback(async () => {
    if (busyRef.current !== null) return;
    const generation = beginOperation("config");
    setActionError(null);
    setNotice(null);
    try {
      const result = await managerApi.applyProviderConfig();
      if (ownsOperation(generation)) setNotice(result.message);
    } catch (cause) {
      if (ownsOperation(generation)) setActionError(resolveFailure(cause, t));
    } finally {
      finishOperation(generation);
    }
  }, [beginOperation, finishOperation, ownsOperation, t]);
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (s.checkOnStartup) void check();
    })();
  }, [check, refreshStatus]);
  ...
  const oneClickConfigButton = () => (
    <button className="btn ghost big" onClick={applyOneClickConfig} disabled={busy !== null}>
      <Icon
        name={busy === "config" ? "loader" : "sliders"}
        className={busy === "config" ? "spinicon" : undefined}
      />
      {busy === "config" ? "正在处理…" : "一键配置"}
    </button>
  );
