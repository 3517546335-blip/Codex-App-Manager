...
function MacHome({ onOpenSettings }: { onOpenSettings: () => void }) {
  ...
  const [busy, setBusy] = useState<"plan" | "perform" | "adopt" | "install" | "config" | null>(
    null,
  );
  ...
  const applyOneClickConfig = useCallback(async () => {
    ...
    setNotice(null);
    try {
      await managerApi.applyProviderConfig();
      setNotice("已写入 DonaAPI 模型配置。");
    } catch (cause) {
        ...
        <div
          className={`actions${
            !rechecking && kind === "update"
              ? " update-actions"
              : !rechecking && (kind === "idle" || kind === "uptodate" || (kind === "error" && installed))
                ? " launch-actions"
                : ""
          }`}
        >
